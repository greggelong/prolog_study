...
put_s(X):-                   %% make it simpler to put object with 1st atribute
	have(object(X,_,_,_)),
	object(X,Y,Z,W), %%  need a call to object(X,Y,Z,W), to get values as it as asserted
	retract(have(object(X,Y,Z,W))),
	here(Place),
	asserta(location_s(object(X,Y,Z,W),Place)),
	write('S Put!'),nl.
put_s(X):-
	write('You don''t have '),
	write(X),nl,
